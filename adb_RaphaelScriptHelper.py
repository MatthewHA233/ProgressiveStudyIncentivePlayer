import adb_ImageProc, ADBHelper, random, time, cv2
import adb_settings as st
...
def find_pic(target, returnCenter = False):
    ...
    if returnCenter == True:
        leftTopPos = adb_ImageProc.locate(st.cache_path + "screenCap.png", target, st.accuracy)
        img = cv2.imread(target)
        centerPos = adb_ImageProc.centerOfTouchArea(img.shape, leftTopPos)
        return centerPos
    else:
        leftTopPos = adb_ImageProc.locate(st.cache_path + "screenCap.png", target, st.accuracy)
        return leftTopPos
...
def find_pic_all(target):
    ADBHelper.screenCapture(deviceID, st.cache_path + "screenCap.png")
    time.sleep(0.1)
    leftTopPos = adb_ImageProc.locate_all(st.cache_path + "screenCap.png", target, st.accuracy)
    return leftTopPos
